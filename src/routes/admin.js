...
async function getWrongProgress() {
  try {
    ...
    issues.push({
      ...
      file: '/api/v1/auth/login',
      issue: 'JWT + Email/Password login (should use API Key + Password)',
      suggestion: 'Deprecate JWT login, use API Key authentication',
      action: 'DEPRECATE'
    });
    ...
    const clean = 45; // Mock value
    const score = Math.round(((clean / (clean + p0 + p1 + p2)) * 100));
